...
export default function DisplayInfo(props) {
  ...
  useEffect(() => {
    setCar((prevCar) => {
      ...
      if (achrayut !== updatedCar["ahrayut"]) {
        ...
        updatedCar["gil"] = gil;
        updatedCar["ahrayut"] = checkDate(updatedCar.moed_aliya_lakvish) || 'אין אחריות';
        updatedCar["memuza_kilometer"] = memuzaKilometerMumlaz(updatedCar.baalut, gil);
      }
      return updatedCar;
    });
  }, [data]);
  ...
  const setInfo = useMemo(
    () =>
      Object.entries(car)
        .filter(([key]) => cardata[key]) // Filter keys based on the presence of corresponding key in cardata object
        .sort(([key1], [key2]) => keyOrder.indexOf(key1) - keyOrder.indexOf(key2)) // Sort keys according to the order in keyOrder array
        .map(([key, value]) => (
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full sm:w-[780px]">
      ...
      <button onClick={handleScrollToTop} className="w-full h-12 bg-[#FFC000] font-bold text-2xl mt-8 border-t-2 border-[#163c39]">חזור למעלה</button>
    </div>
  );
}